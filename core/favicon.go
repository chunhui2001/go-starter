...
func Favicon(path string) gin.HandlerFunc {
	...
	if ok, _ := utils.FileExists(path); ok {
		...
		if err != nil || info == nil || info.IsDir() {
			...
			if err != nil {
				logger.Warnf(`Read-Favicon-Path-Error: path=%s, errorMessage=%s`, path, err.Error())
				reader = bytes.NewReader(faviconBuf)
			} else {
...
// func isFresh(c *gin.Context) bool {

// 	modifiedSince := c.Request.Header["if-modified-since"]
// 	noneMatch := c.Request.Header["if-none-match"]

// 	if (modifiedSince == nil && noneMatch == nil) ||
// 		(utils.ToString(modifiedSince) == "" && utils.ToString(noneMatch) == "") {
// 		return false
// 	}

// 	var cacheControl = c.Request.Header["cache-control"]

// 	if cacheControl != nil && strings.Contains(utils.ToString(cacheControl), "no-cache") {
// 		return false
// 	}
